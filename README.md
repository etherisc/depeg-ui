...
1. Trigger a depeg event by calling an HTTP PUT to `{{ _.monitor_base_url }}/v1/feeder/set_state/triggered`
1. Wait a bit for the price feed to go into triggered state (how long to wait depends in the price feed config)
1. Trigger a depeg event by calling an HTTP PUT to `{{ _.monitor_base_url }}/v1/feeder/set_state/depegged`
...
1. If you want to reset the system to be able to issue new policies, call an HTTP PUT to `{{ _.monitor_base_url }}/v1/product/reactivate`
